...
<script>
    ...
    var productVersionId = "<?php echo isset($productVersionId) ? $productVersionId : ''?>";
</script>

@extends('layout.header')
@section('title', 'Modifica Versione')
@section('content')
@include('layout.menu')
<input type="hidden" id="trademarkid">
        ...
        <div class="medium-12 large-12 columns">
            <nav aria-label="You are here:" role="navigation">
                <ul class="breadcrumbs">
                    ...
                    <li class="disabled">Modifica Versione</li>
                </ul>
            </nav>
        </div>
    </div>
     ...
     <div class="row">
        <div class="medium-12 large-12 columns">
            <div class="scheda-nuovo-prodotto">
                <form class="modifica_versione" method="post">
                    <div class="row">
        ...
        <div class="medium-12 large-12 columns">
            <div class="contenitore-tab-stato-codice-prodotto contenitore-tab-stato-codice-prodotto-duplica">
                <ul class="tabs" data-tabs id="stato-codice-prodotto-tabs">
                    ...
                    <li class="tabs-title"><a href="#panel2">Nuovo Artwork</a></li>
                </ul>
                <div class="tabs-content" data-tabs-content="stato-codice-prodotto-tabs">
                    ...
                    <div class="tabs-panel" id="panel2">
                        <div class="form-carica-documenti">
                            <form class="upload_doc_art" method="post">
                                <div class="row">
                                    <div class="large-3 columns">
                                        <label>Scegli file</label>
                                    </div>
...
<script type="text/javascript">
    ...
    $.ajax({
        ...
        success: function (result){
            ...
            /*$.each(result.data.regulatory.adviceClp, function(index, value){ 
                adviceClp.push(value.id); 
            });*/
            //console.log(adviceClp);
            ...
            $.ajax({
                ...
                success: function (result) {
                    $.each(result.data, function(index, value){
                        ...
                        avvertenze = avvertenze + '<div class="large-3 columns"><input type="radio" name="Pericolo" ' + (index == 0 ? 'checked="chedked"' : '') + ' value="' + value.id + '"><span>' + value.name + '</span></div>';
                    });
    ...
    $.ajax({
        ...
        success: function (result) {
            $.each(result.data, function(index, value){ 
                ...
                //var selected = $('#trademarkid').val() == value.id ? 'selected' : '';
    ...
    $.ajax({
        ...
        url: "{{ config('constants.UrlWebService') }}api/Products/GetCodiceDetail?lcode=" +  lcode + "&companyId=" +  companyId + "&version=" + version,
    ...
    $('.upload_doc_min').on('submit', function (e){
        ...
        myObj = { "CreatedBy": utente , "FileType": {"ID": 2}, "ProductCodeVersionId": productVersionId};
        ...
        $.ajax({
            ...
            url: "{{ config('constants.UrlWebService') }}api/Upload/UploadFileEtichettaToVersion",
            contentType: false,
            processData: false,
            data: data,
            success: function (result) {
                if(result.returnCode == '-1'){
                    ...
                    $('.upload_doc_art').trigger('reset');
                    $('.upload_doc_art span.filupp-file-name').html('');
    ...
    $('.upload_doc_art').on('submit', function (e){
        e.preventDefault();
        ...
        myObj = { 
            ...
            "ProductCodeVersionId": productVersionId
        };
        myJSON = JSON.stringify(myObj);
        data.append("data", myJSON);
            ...
            success: function (result) {
                if(result.returnCode == '-1'){
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        data: JSON.parse(post_data),
                        success: function (result){ $("#error").html('Artwork correttamente caricata'); },
                        error: function (request, status, errorThrown){ $("#error").html('Impossibile caricare il file'); }
                    });
                }
            },
    ...
    $(document).on('submit', '.modifica_versione', function(e) {
        e.preventDefault();
        var frasi_h = Array();
        ...
        $.ajax({
            ...
            url: "{{ config('constants.UrlWebService') }}api/Products/UpdateCodeVersion",
            dataType: "json",
            data: {
                'Id': productVersionId,
                'BarcodeImballoSecondario': $('#barcode_imballo_secondario').val(),
                'DataTracciabilita': $('#data_tracciabilita').val(),
                'DeadlineOwner': $('#deadline_owner').val(),
                'DeadlineSeller': $('#deadline_seller').val(),
            ...
            success: function (result) {
                if(result.returnCode != 0 ) { 
                    $("#error").html('Impossibile modificare la versione: ' + result.errors);
                } else { 
                    $("#error").html('Versione correttamente modificata'); 
                }
                $("html, body").animate({ 
                    scrollTop: 0 
                }, "slow");
            ...
            error: function (request, status, errorThrown) {
               $("#error").html('Impossibile modificare la versione.');
