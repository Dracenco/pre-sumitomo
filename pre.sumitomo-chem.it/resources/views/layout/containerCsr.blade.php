        ...
        success: function (results){
            $.each(results.data, function(index, value){
                if(value.idSezione == 34){  
                  $("#catalogoSiapa").html('<a href="{{ config('constants.UrlWebService') }}api/Site/DownloadImmagine?IdImmagine=' + value.id + '" target="_blank" title="Scarica il catalogo Siapa" class="siapaCatalogo"><span class="catalogo-scarica_siapa"></span><span>Catalogo<br>Siapa</span></a>');
                  $("#catalogoSiapaFascia").html('<div><a href="{{ config('constants.UrlWebService') }}api/Site/DownloadImmagine?IdImmagine=' + value.id + '" target="_blank" title="Scarica il catalogo completo Siapa" class="siapaCatalogo link"><span><i class="catalogo-scarica_siapa"></i></span>Catalogo<br>Siapa</a></div>');
                }
            });
        }
    });
